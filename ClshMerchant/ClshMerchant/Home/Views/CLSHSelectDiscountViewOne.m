...
@interface CLSHSelectDiscountViewOne ()<UITableViewDelegate,UITableViewDataSource>{
...
@implementation CLSHSelectDiscountViewOne

#pragma mark -- 懒加载
- (UITableView *)tableView{
    
    if (!_tableView) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, 100*AppScale,100*AppScale) style:(UITableViewStylePlain)];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.selectDiscountblockOne) {
        self.selectDiscountblockOne(indexPath.row);
    }
}
